...
- TRATAR exit code 4 como memória não encontrada
...
- `reclassify` retorna `action` ("reclassified"), `count`, `namespace`, `elapsed_ms`
- `merge-entities` retorna `action` ("merged"), `sources[]`, `target`, `namespace`, `relationships_moved`, `entities_removed`, `elapsed_ms`
- `memory-entities` retorna `memory_name`, `entities[].{entity_id, name, entity_type}`, `count`, `elapsed_ms`
