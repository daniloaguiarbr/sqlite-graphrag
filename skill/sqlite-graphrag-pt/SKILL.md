...
description: Use esta skill SEMPRE que o usuário perguntar sobre adicionar memória persistente ou GraphRAG ou contexto de longo prazo ao Claude Code Codex Cursor Windsurf ou qualquer agente de IA de código. DEVE acionar para queries mencionando lembrar disso, salvar conversa, recuperar contexto anterior, busca híbrida, grafo de entidades, memória SQLite, RAG local, embedding LLM-only, fluxo OAuth, embedding BLOB-backed, migração v1.0.76, migrate to-llm-only, migrate rehash, drop de vec tables, helper codex-spawn, tratamento de órfãos vec, ou qualquer remediação de gap G28-G39. Auto-invoca mesmo sem menção explícita quando usuário descreve problema de agente perdendo contexto entre sessões ou quer camada de memória local offline-first em Rust. DEVE também acionar em enforcement OAuth-only, aborto de ANTHROPIC_API_KEY ou OPENAI_API_KEY, 7 flags de endurecimento para Claude e Codex, Mock LLM CLI em CI, Mock LLM CLI em CI, ou remoção do subcomando daemon. Keywords memória RAG GraphRAG SQLite LLM-only one-shot OAuth Claude Codex Cursor Windsurf offline local persistente grafo entidade v1.0.76.
---
...
- NUNCA depender do subcomando `daemon` para código novo — ele é removido na v1.1.0
...
## Nota de Arquitetura — Sem Daemon (v1.0.76)
