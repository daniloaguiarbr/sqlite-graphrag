...
- Input `--type <kind>` aceita `user`, `feedback`, `project`, `reference`, `decision`, `incident`, `skill`, `document` ou `note` (9 tipos de memória).
...
- Input `--max-concurrency <N>` limita invocações simultâneas da CLI em comandos pesados; o valor DEVE estar em `[1, 2*nCPUs]`. Default 4. Valores fora do intervalo retornam exit `2`. Para auditorias em corpora grandes comece com `--max-concurrency 1`.
- Input `--wait-lock <SECONDS>` aguarda até N segundos por slot livre antes de sair com exit `75`. Default 300 (5 minutos).
...
- `list --json` retorna `{items: [{memory_id, id, name, namespace, type, description, snippet, updated_at, updated_at_iso}], elapsed_ms}` (objeto envolvendo o array, NÃO um array de raiz).
