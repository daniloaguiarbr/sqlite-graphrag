...
- Divide a query em até 7 sub-queries, executa em paralelo via JoinSet + Semaphore bounded
...
- Desde v1.0.64: `ingest --mode claude-code` desabilita hooks para OAuth via `--settings '{"hooks":{}}'`, detecta OAuth para omitir `cost_usd` enganoso, valida tamanho do body antes da extração LLM (arquivos >512 KB ignorados) e `rename`/`rename-entity` rejeitam mesmo nome com exit 1
