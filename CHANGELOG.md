...
### Fixed
- OpenRouter key preflight no longer hard-fails read-only / no-embedding subcommands: the eager `--embedding-backend openrouter` guard in `main` returned exit 78 for *every* subcommand when no API key resolved, including `init` (schema only — it already self-degrades to `ok_no_embedding`) and the `enrich` queue inspectors (`--status`/`--list-dead`/`--requeue-dead`/`--prune-dead-orphans`, which never embed). New `Commands::tolerates_missing_embedding_key()` scopes the guard so these run without a key; `remember`/`recall`/`hybrid-search`/`ingest`/`deep-research` keep failing fast. This was the deterministic cause of the ubuntu/macOS test-job failures (`tests/enrich_queue_db_isolation.rs` asserts a keyless `init` succeeds).
- Security advisory `RUSTSEC-2026-0190` (unsoundness in `anyhow::Error::downcast_mut()`): `anyhow` bumped 1.0.102→1.0.103 in `Cargo.lock`, clearing `cargo audit` and `cargo deny check advisories`.
- English-only doc policy: `///`/`//!` doc comments across `src/` and `tests/` that still carried Portuguese (the source of the `language-check` job failure) are translated to English; comments only, no behaviour change.

### CI / infrastructure
- Windows runners: four `Pre-warm`/`Verify` steps declared `shell: pwsh` but their bodies mixed a bash `for … do … done` header with PowerShell `if`/`Start-Sleep`, so PowerShell rejected the loop and the windows-2025 clippy/test jobs died before running. Converted to `shell: bash` with the retry idiom already proven in the test job.
- SemVer gate (G53): pinned `cargo-semver-checks` to 0.44.0 via `taiki-e/install-action` (prebuilt binary); the unpinned `cargo install` pulled 0.48.0, which needs rustc 1.91 > the project MSRV 1.88 and failed to compile. Baseline bumped 1.0.79→1.0.96.
- Windows MSVC cross-check (G29): the `rust-toolchain.toml` `channel = "1.88"` override made `cargo` use 1.88 while the windows-msvc target had been added to `stable`, failing with `error[E0463]: can't find crate for 'core'`. The target is now added to the active (overridden) toolchain; this exposed that `ring` (via reqwest+rustls) needs the MSVC compiler, so the cross type-check now runs through `cargo-xwin` (LLVM MSVC sysroot). Verified locally on Fedora with `cargo xwin check --target x86_64-pc-windows-msvc --lib --all-features`.
- Tags: the divergent `v1.2.0`/`v1.2.1`/`v2.0.0`..`v2.0.5`/`v2.1.0`/`v2.2.0`/`v2.3.0` tags (a parallel lineage whose commits are not reachable from the 1.0.x line) were removed from the remote so the GitHub Releases view tracks the real version line.


## [1.0.97] - 2026-06-29

This release closes the 56-gap backlog (`GAP-SG-01`..`GAP-SG-56`) catalogued in `gaps.md` from the real ingest/enrich of the rules-rust corpus. Work landed across 5 commits: `eeb40d5` (Phase 0 foundation), `aaeebcc` (Phase A), `a67b863` (Phases B + C-F), `dc6b974` (Phases G + J + M), `f418957` (Phases H + I + K + L).

### Added
- Phase 0 foundation (`eeb40d5`): new dependencies `llm_json` + `tiktoken-rs`, module `src/json_repair.rs`, and `count_tokens` helper underpinning the resilience layer below
- Resilient OpenRouter HTTP layer (`aaeebcc`, `GAP-SG-01/03/56`): the embedding/chat REST parse branches by inspecting response fields (`Option<data>`/`Option<error>` struct) instead of optimistic parsing; an HTTP 200 carrying `{error}` (token overflow) now propagates the real `code`/`message` instead of `missing field data`; the server `Retry-After` on 429 is surfaced to the caller via `RateLimited`
- Embedding, tokens and chunking (`a67b863`, `GAP-SG-02/04/05/06/07`): token guard `EMBEDDING_REQUEST_MAX_TOKENS=30000` at the HTTP boundary (distinct from the per-chunk `EMBEDDING_MAX_TOKENS=512`); `estimate_chunk_count` + `assess_body_budget` so `--dry-run` reports chunk/token/partition counts; native lossless auto-split by markdown section into sub-memories under the bytes/chunks/tokens limits
- Enrich resilience and dead-letter recovery (`a67b863`, `GAP-SG-08`..`16`/`18`/`19`/`21`..`28`/`42`/`45`/`46`): JSON repair via `json_repair::repair_to_value` plus a shape guard before strict parse; non-JSON reclassified `HardFailure`→`Transient`; `--max-attempts` default 5→8; `enrich --requeue-dead` (dead→pending) distinct from `--retry-failed`; `enrich --list-dead` with `error_class`/`message`; `waiting_items[]` exposing per-item `next_retry_at` plus `--ignore-backoff`; queue keyed by `memory_id` with idempotent `operation` column migration; cascade `cleanup_queue_entry` on forget/purge/force-merge; status now reports waiting+dead, per-operation counts, scan-vs-drain parallelism and pending-scan state; new `augment-bindings` operation for already-bound memories filtered by `--names`; `body-extract` honours `--names`; `--body-extract-graph-only` read-only mode; sidecar/`--names` documented in help
- Clap parsing fixes (`dc6b974`, `GAP-SG-29/30/31/33/34/35` + `GAP-SG-17`): `enrich --status/--list-dead/--requeue-dead` no longer demand `--operation`/`--mode` (`required_unless_present_any`); `remember --graph-file` combinable with `--body-file` (separate fd); `allow_hyphen_values` on `--description`/`--body`; `--json` on every `config` variant (`config doctor --json`); `--llm-parallelism` declared on `remember-batch`; `ingest --mode none --resume` now fails fail-fast before any IO; `--openrouter-timeout` default raised 300→600 for dense bodies
- Canonical graph vocabulary (`dc6b974`, `GAP-SG-47/48/49`): `EntityType::map_to_canonical` maps non-canonical types (`platform`/`language`/`feature`→`concept`) instead of dropping them, with the canonical list injected into the extraction prompt; `map_to_canonical_relation` unifies LLM-extracted relations (`part-of`→`applies-to`); `graph::enforce_degree_cap` is now actionable (prunes the lowest-weight edge until degree ≤ cap), wired into `link.rs`/`remember.rs`
- Names and write observability (`f418957`, `GAP-SG-37/38/39`): `remember --strict-name` rejects a non-kebab name returning the canonical form; truncation warning promoted debug→warn with `truncated`/`original_name` in the NDJSON; `AppError::suggestion()` emits `{error,code,message,suggestion}` on any non-zero write
- Read, merge and prune (`f418957`, `GAP-SG-50/51/52`): `read --format raw` emits the raw body without an envelope; `remember --replace-graph` clears bindings before re-linking (`entities:[]` clears without `forget`); `unlink --memory --entity` removes a curated binding that `prune-ner` cannot reach
- Inventory and ingest (`f418957`, `GAP-SG-53/54/55`): `list --json` emits a `truncation_warning` recommending `export` as the reliable inventory; `ingest --force-merge` updates duplicates in-place; `ingest` deduplicates by `body_hash` so divergent naming no longer duplicates content

### Fixed
- Metrics that lied are now truthful (`a67b863`/`f418957`, `GAP-SG-40/41/43/44`): `chunks_persisted` reads the real post-commit COUNT (`storage_chunks::count_for_memory`); `embedding status` reports a coverage object of real vectors in the tables instead of the always-empty async queue; `total_memories` is populated in `stats --json`; `remember` checks the vector post-commit and recommends `re-embed` when absent
- Token-budget guard was measured in bytes while the real `qwen/qwen3-embedding-8b` limit is ~32K tokens; large bodies now fail predictably (or auto-split) before the network call (`GAP-SG-02`)
- Enrich dead-letter classified probabilistic schema misses as permanent `HardFailure` on first failure, killing recoverable items; non-JSON is now `Transient` with an 8-attempt budget plus source-side repair, and `dead` items are recoverable via `--requeue-dead` (`GAP-SG-08/09/10/11/14/21`)

### Post-sealing audit (working tree, GAP-SG-57..66)

The end-to-end audit that followed the 56-gap sealing surfaced a tech-debt block, closed in the working tree on top of the five commits above:
- Enrich modularisation + `unwrap`/`expect` audit + `parse_claude_output` DRY (`ADR-0056`, `GAP-SG-57/58/59/60`): `src/commands/enrich.rs` (6013 lines) split into the directory module `src/commands/enrich/` (`mod.rs` 2355 + `queue` + `scan` + `postprocess` + `extraction`), the six public symbols (`run`, `EnrichArgs`, `EnrichOperation`, `EnrichMode`, `EnrichStatus`, `cleanup_queue_entry`) preserved and 36 enrich tests intact; the real production `unwrap`/`expect` count was ~36 (not the audited 423, which counted `#[cfg(test)]`), all converted to `?`/`ok_or_else`/poison-recovery and gated by `#![cfg_attr(not(test), warn(clippy::unwrap_used, clippy::expect_used))]` in `src/lib.rs` (the gate revealed 5 more in `config_cmd.rs`); `claude_runner::parse_claude_output_opts(stdout, tolerate_max_turns)` removes the `parse_claude_output` duplication while preserving the `max_turns` divergence (guarded by `test_terminal_reason_max_turns_detected`)
...
- Queue CWD-relative bug class fixed (`ADR-0057`, `GAP-SG-64` enrich + `GAP-SG-65` ingest): the worklist sidecars (`.enrich-queue.sqlite`, `.ingest-queue.sqlite`) resolved against the process CWD instead of the `--db` directory, so `enrich --status` reported the wrong queue when `--db` diverged from the CWD and the ingest `--resume`/`--retry-failed` lost the queue across CWD changes; new helper `paths::sidecar_path(db_path, filename)` derives the sidecar next to the database (graceful CWD fallback for the default DB), the enrich `DEFAULT_QUEUE_DB` const is removed and `cleanup_queue_entry` gains a leading `db_path` parameter, `IngestArgs.queue_db` becomes `Option<String>` with no clap default; regression test `tests/enrich_queue_db_isolation.rs` plants a queue next to `db_a` and proves `--status` reads it from an unrelated CWD; no legacy file migration (the canonical default coincides with the legacy `./.enrich-queue.sqlite`)
- Cleanup: removed the dead `constants::CLI_LOCK_FILE` const (zero uses; the real lock uses `lock.rs` with the XDG-derived `cache_dir()`)
- Hook audit + orphan dead-letter cleanup (`ADR-0058`, `GAP-SG-66`): a Claude Code hook audit found `lib/graphrag-recover-dead.sh` calling `pending list --filter-status dead` (rejected by v1.0.97 — `dead` is not a `--filter-status` value and `pending list` takes no `--namespace`); fixing it exposed orphan dead-letter rows (memory renamed/purged after enqueue — 110 in the real DB, all `permanent` "not found") that `--requeue-dead` only re-fails and no command could drop. New read-only inspector `enrich --prune-dead-orphans` (in the `required_unless_present_any` group) deletes ONLY `status='dead' AND item_type='memory'` rows whose `item_key` is absent from the main DB, reusing the `enqueue_candidate` existence query; `DeadSummary` gains a `pruned` field (schema-neutral — not a dumped struct). Hooks rewired: `recover-dead.sh` uses `--list-dead` + `--prune-dead-orphans`, the worker residual now emits db-scoped `total_dead` (reliable since `ADR-0057`, un-breaking the `auto-enrich.sh`/`memory-guardian.sh` consumers — GAP-B), and `GR_OPS_GATE`/`gr_dead_total`/`gr_prune_orphans` are centralised in `graphrag-common.sh`; unit test `prune_dead_orphans_removes_only_orphan_memory_rows` + real smoke pruned 110 (`dead_total` 110→0, `pruned:110`)

### Audit Notes
- Build clean: 0 errors; `cargo clippy --all-targets -- -D warnings` 0 warnings; `cargo fmt --check` 0 diffs
- Test suite: default `cargo test` 1164 passed / 0 failed; `cargo test --features slow-tests` 1522 passed / 0 failed / 11 ignored at sealing time (the `installed_binary_smoke` suite was skipped while `~/.cargo/bin` still held the stale 1.0.96 binary); after the post-sealing work `cargo install --path . --locked --force` realigned the global binary to 1.0.97 and `installed_binary_smoke` now runs 26/0 WITHOUT bypass (GAP-SG-62 resolved), with `cargo test --lib` 973/0 and the new regression tests (`paths::sidecar_path` ×3, `tests/enrich_queue_db_isolation.rs` ×1) green; `cargo fmt --check` 0 diffs; `cargo clippy --all-targets --features slow-tests -- -D warnings` 0 warnings
- Test contracts synced to the new output fields: `docs/schemas/stats.schema.json` gains `total_memories` (GAP-SG-43) and `docs/schemas/enrich-summary.schema.json` gains `dead`/`waiting` (GAP-SG-15/16), keeping the strict `schema_contract_strict` suite green; new integration test `test_read_format_raw_emits_pure_body` in `tests/integration.rs` asserts the GAP-SG-50 raw-stdout contract end-to-end
- `gaps.md` updated: all 56 `GAP-SG-NN` entries carry a resolution STATUS referencing the landing commit; `GAP-SG-20` stays by-design (`--rest-concurrency` intra-batch is the throughput path) and `GAP-SG-36` is verified (the effective hook already allows `--help`)
- `GAP-SG-32` is functionally resolved (`--db` after the subcommand + `SQLITE_GRAPHRAG_DB_PATH`); only its doc note is deferred
