...
//! 2. Env var `NEUROGRAPHRAG_DISPLAY_TZ`
//! 3. Fallback UTC
//!
...
/// Resolve o fuso a partir do env var `NEUROGRAPHRAG_DISPLAY_TZ`.
///
...
fn resolver_tz_de_env() -> Result<Tz, AppError> {
    match std::env::var("NEUROGRAPHRAG_DISPLAY_TZ") {
...
/// Se `explicit` for `None`, tenta `NEUROGRAPHRAG_DISPLAY_TZ`, depois UTC.
///
...
mod testes {
    ...
    fn utc_default_quando_env_ausente() {
        ...
        std::env::remove_var("NEUROGRAPHRAG_DISPLAY_TZ");
    ...
    fn env_valido_aplica_timezone() {
        std::env::set_var("NEUROGRAPHRAG_DISPLAY_TZ", "America/Sao_Paulo");
    ...
    fn env_invalido_retorna_erro_validation() {
        std::env::set_var("NEUROGRAPHRAG_DISPLAY_TZ", "Invalido/Naoexiste");
        let resultado = resolver_tz_de_env();
        assert!(resultado.is_err(), "timezone inválida deve retornar Err");
        match resultado {
            Err(AppError::Validation(msg)) => {
                assert!(
                    msg.contains("NEUROGRAPHRAG_DISPLAY_TZ"),
