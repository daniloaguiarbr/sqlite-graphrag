...
> **Current release: v1.0.78 — LLM-Only and one-shot.** The default build embeds through `claude -p` or `codex exec` (OAuth, no MCP, no hooks). No daemon, no ONNX runtime, ~6 MB binary. To embed locally, install with `--features embedding-legacy`.
...
- **Default build (v1.0.78):** LLM-only and one-shot — embeddings are generated by spawning `claude -p` or `codex exec` with OAuth; no local model, no daemon, no ONNX runtime, ~6 MB binary
...
- **Default build (v1.0.78):** each embedding call spawns `claude -p` or `codex exec` — RSS is ~350 MB per LLM worker
