...
- **Build (v1.0.79):** LLM-only and one-shot — embeddings are generated by spawning `claude -p` or `codex exec` with OAuth; no local model, no daemon, no ONNX runtime, ~6 MB binary
...
| `SQLITE_GRAPHRAG_CLAUDE_BINARY` | Explicit path to the Claude Code binary for `ingest --mode claude-code` | PATH lookup | `/usr/local/bin/claude` |
| `SQLITE_GRAPHRAG_CODEX_BINARY` | Explicit path to the Codex CLI binary for `ingest --mode codex` | PATH lookup | `/usr/local/bin/codex` |
...
- The CLI itself is lightweight (~6 MB binary); RAM is dominated by the LLM subprocesses at roughly 350 MB RSS per worker (`LLM_WORKER_RSS_MB`)
