...
> **Current release: v1.0.77 — LLM-Only and one-shot.** The default build embeds through `claude -p` or `codex exec` (OAuth, no MCP, no hooks). No daemon, no ONNX runtime, ~6 MB binary. To embed locally, install with `--features embedding-legacy`.
...
- **Upgrading from v1.0.74 / v1.0.75?** See [docs/MIGRATION.md](docs/MIGRATION.md) for the v1.0.76 / v1.0.77 migration procedure.
...
- **Default build (v1.0.77):** LLM-only and one-shot — embeddings are generated by spawning `claude -p` or `codex exec` with OAuth; no local model, no daemon, no ONNX runtime, ~6 MB binary
- **Legacy build:** `cargo install sqlite-graphrag --features embedding-legacy` restores local ONNX inference via fastembed (~39 MB binary)
...
- **GraphRAG is enabled by default and runs automatically.** Every subcommand auto-initializes `graphrag.sqlite` in the current working directory if it does not exist. `remember` and `ingest` can extract entities and relationships via local GLiNER zero-shot NER when `--enable-ner` is passed.
...
- **Default build (v1.0.77):** each embedding call spawns `claude -p` or `codex exec` — RSS is ~350 MB per LLM worker
- **Legacy build:** embedding model uses approximately 1100 MB of RAM per process when using `--features embedding-legacy`
