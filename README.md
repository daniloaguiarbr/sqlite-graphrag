...
> **Current release: v1.0.95 — `enrich --mode openrouter` routes the extraction JUDGE through the OpenRouter REST chat API, so structured extraction no longer needs a local claude/codex/opencode CLI.** Every build embeds through `claude -p`, `codex exec`, `opencode run` (OAuth) or OpenRouter REST API (`--embedding-backend openrouter`). No daemon, no ONNX runtime, ~14.6 MiB binary. v1.0.94 adds `--embedding-backend auto|openrouter|llm` with `--embedding-model` for REST API embeddings (~200ms vs 15s subprocess LLM), propagates `EmbeddingBackendChoice` to ALL 13 embedding paths (GAP-OR-PROPAGATION), fixes exit code 78 for OpenRouter config errors (BUG-OR-EXIT-CODE), and verifies 10 embedding models E2E. The prior OpenRouter embedding backend (`--embedding-backend openrouter`) is unchanged. Library consumers must pin to `=1.0.95`; see the `Stability Policy` below.
