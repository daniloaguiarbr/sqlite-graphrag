...
> **Current release: v1.1.01 — closes the 12-priority `gaps.md` roadmap from the production-database audit (the crate manifest carries `version = "1.1.1"` because SemVer rejects a leading zero in the patch component): P1 routes entity embedding through the OpenRouter REST API even under `--llm-backend none` (chain `[OpenRouter]`) and adds an empty-vector guard to the memory/entity/chunk vector upserts; P2 adds retroactive backfill via `enrich --operation re-embed --target memories|entities|chunks|all` plus a per-target `scan_backlog` in `enrich --status`; P3 adds the `graph recompute-degree` subcommand (single transaction, `--dry-run`, envelope `{total, updated, zeroed, unchanged}`); P4 adds `reclassify-relation --literal-from`, matching the stored relation verbatim (no clap normalization) so legacy hyphenated edges become migratable; P5 adds `merge-entities --ids/--into-id` and `rename-entity --id` for namespace-scoped ID disambiguation; P6 adds `vec_memories_missing`/`vec_entities_missing`/`vec_chunks_missing` plus per-table `vec_*_coverage_pct` to `health --json` and per-table `*_missing` counters to `embedding status --json`; P7 documents the canonical vocabularies and gives `EntityType` a manual `Deserialize` so an invalid value in `--graph-stdin` fails early listing the 13 valid values; P10 makes the `re-embed` predicates also select divergent-`dim` or empty-blob vectors, not only missing ones; P11 adds the typed `AppError::BodyTooLarge`/`AppError::TooManyChunks` variants carrying bytes/chunks and the limit in the envelope (exit 6 preserved); P12 adds `ingest --name-prefix` with name-cap validation. The HTTP User-Agent derives from CARGO_PKG_VERSION (`sqlite-graphrag/1.1.1`). Schema stays at v15, no migration. Previous release v1.1.0 — resolves the enrichment dead-letter backlog at its root: GAP-SG-70 retries truncated OpenRouter completions (`finish_reason=length`) with a grown `max_tokens`; GAP-SG-71 adds adaptive `max_tokens` constants; GAP-SG-72 adds dead-letter diagnostic columns (`finish_reason`/`input_tokens`/`output_tokens`, via `--list-dead --json`); GAP-SG-73 makes retry-classification fully typed (no message-substring), so an exhausted-internal-retry is `Transient` not an instant dead-letter; GAP-SG-74 extracts the shared `openrouter_http` module; GAP-SG-75 bumps the HTTP User-Agent to `sqlite-graphrag/1.1.0`; GAP-SG-76 bounds the dequeue under lock contention (exit 15 on sustained `SQLITE_BUSY` instead of a false empty backlog); GAP-SG-77 makes `enrich --status` report a real per-operation `scan_backlog` (killing the false `pending=0` for `entity-descriptions`/`body-enrich`/`re-embed`, with `state` derived from it); GAP-SG-78 classifies a not-yet-materialized entity as `Transient` via a typed `AppError::EntityNotYetMaterialized` and fixes the namespace-blind `entity-type-validate` lookup. Schema stays at v15 (idempotent ALTER). Previous release v1.0.99 — GAP-SG-67 removed the destructive global degree-cap pruning and the `--max-entity-degree` flag from `remember`/`link` (BREAKING: clap exit 2 if passed; writes are now purely additive and never delete edges); GAP-SG-68 aligned the `graph entities --sort-by degree` doc to its ascending behaviour (`--order desc` for most-connected-first); GAP-SG-69 `enrich --operation body-enrich --until-empty` now converges. Previous release v1.0.97 — enrich dead-letter recovery and queue inspectors (`--requeue-dead`, `--list-dead`, `--ignore-backoff`, `--prune-dead-orphans`), `--status`/`--prune-dead-orphans` without `--operation`/`--mode`, the `augment-bindings` operation, `remember --graph-file/--strict-name/--replace-graph`, `ingest --force-merge` with `body_hash` dedup, `read --format raw`, and `unlink --memory/--entity` (GAP-20, SG-32).** Since v1.0.95, `enrich --mode openrouter` routes the extraction JUDGE through the OpenRouter REST chat API, so structured extraction no longer needs a local claude/codex/opencode CLI. Every build embeds through `claude -p`, `codex exec`, `opencode run` (OAuth) or OpenRouter REST API (`--embedding-backend openrouter`). No daemon, no ONNX runtime, ~19 MiB binary. v1.0.94 adds `--embedding-backend auto|openrouter|llm` with `--embedding-model` for REST API embeddings (~200ms vs 15s subprocess LLM), propagates `EmbeddingBackendChoice` to ALL 13 embedding paths (GAP-OR-PROPAGATION), fixes exit code 78 for OpenRouter config errors (BUG-OR-EXIT-CODE), and verifies 10 embedding models E2E. The prior OpenRouter embedding backend (`--embedding-backend openrouter`) is unchanged. Library consumers must pin to `=1.1.1`; see the `Stability Policy` below.
...
| `merge-entities` | `--names <a,b,c> --into <target>`, or `--ids <1,2,3> --into-id <ID>` (v1.1.01, namespace-scoped) | Merge source entities into target, moving all edges |
...
| --- | --- | --- | --- |
| `SQLITE_GRAPHRAG_DB_PATH` | Path to the SQLite database file override; this is the canonical, position-independent override. The `--db <PATH>` flag is equivalent but must be placed AFTER the subcommand (e.g. `remember --db <PATH>`) (SG-32) | `./graphrag.sqlite` in the invocation directory | `/data/graphrag.sqlite` |
