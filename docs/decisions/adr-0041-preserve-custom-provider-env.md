...
O `sqlite-graphrag` v1.0.76+ bloqueia uso de providers customizados (MiniMax/api.minimax.io, OpenRouter, gateways corporativos) porque os três spawners de LLM aplicam `env_clear()` e depois injetam apenas um whitelist restrito que NÃO inclui `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `OPENAI_BASE_URL` nem outras variáveis de provider customizado.
