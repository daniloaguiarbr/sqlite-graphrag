## Novidades na v1.0.83
### OBRIGATÓRIO — Preservação de Credenciais de Provider Customizado (ADR-0041)
- Seis vars de ambiente de provider customizado agora são preservadas ao spawnar subprocessos `claude -p` ou `codex exec`. As vars preservadas são `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `OPENAI_BASE_URL`, `CLAUDE_CODE_ENTRYPOINT`, `DISABLE_TELEMETRY` e `OTEL_EXPORTER_OTLP_ENDPOINT`. Habilita providers compatíveis com Anthropic (Minimax/api.minimax.io, OpenRouter, AWS Bedrock, gateways corporativos) sem alterar o mandato OAuth-only que continua rejeitando `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`
- O guard OAuth-only em `claude_runner.rs:273`, `codex_spawn.rs:259`, `ingest_claude.rs:282` e `extract/llm_embedding.rs:237-253` é preservado; a mensagem de erro de abort agora referencia `ANTHROPIC_AUTH_TOKEN` e `~/.codex/auth.json` como resoluções legítimas quando um operador seta `ANTHROPIC_API_KEY` por engano
- Novo helper compartilhado `src/spawn/env_whitelist.rs` expõe `apply_env_whitelist(cmd, strict)` e `is_strict_env_clear()`. Os três spawners (`claude_runner`, `codex_spawn`, `ingest_claude`) delegam em vez de inlinear o array, eliminando o drift entre whitelists duplicados
- Nova flag global `--strict-env-clear` / `SQLITE_GRAPHRAG_STRICT_ENV_CLEAR=1` ativa modo estrito que preserva apenas `PATH`. Use em ambientes PCI-DSS, SOC2, HIPAA onde encaminhamento de credenciais via env vars é proibido por política. Padrão permanece permissivo (encaminha as seis vars de custom-provider)
...
- 6 novos testes de regressão em `tests/claude_runner_env.rs` cobrem: propagação de custom-provider, preservação do abort OAuth-only, herança de base-URL codex, drop de credenciais em modo estrito, auditoria no-leak, e um cenário documentado intencionalmente vazio (teste de env claude) com caminho de integração equivalente coberto para codex. Todos com `#[serial_test::serial(env)]`
...
- Embedding via HTTP direto (~200ms) em vez de subprocesso LLM headless (~15-20s por chamada)
- O usuário DEVE especificar `--embedding-model` ao usar OpenRouter — NÃO há modelo padrão
- Defina a chave de API via var de ambiente `OPENROUTER_API_KEY` ou flag `--openrouter-api-key`
...
- Nova flag `--enrich-after` no `ingest` — executa `enrich --operation memory-bindings` sequencialmente após ingestão
- 10 modelos OpenRouter verificados E2E: qwen3-embedding-4b, qwen3-embedding-8b, llama-nemotron-embed-vl-1b-v2:free, text-embedding-3-small, text-embedding-3-large, pplx-embed-v1-0.6b, mistral-embed-2312, bge-m3, gemini-embedding-001, gemini-embedding-2
- 5 correções de bug: BUG-OR-1 (input_type por modelo), BUG-OR-2 (detecção MRL), BUG-OR-3 (validação de modelo), BUG-OR-4 (retry em HTTP 200 malformado), BUG-OR-5 (override de dimensão)
- Veja ADR-0052 para arquitetura; `gaps.md` para tabelas causa-efeito
...
- O caminho de embedding usa `CLAUDE_CONFIG_DIR` VAZIO por padrão (G42/S6): honra `SQLITE_GRAPHRAG_CLAUDE_EMPTY_CONFIG_DIR`, senão um diretório gerenciado `~/.local/state/sqlite-graphrag/claude-empty-config`; as flags de isolamento MCP são silenciosamente ignoradas pelo upstream (anthropics/claude-code#10787) e um `~/.claude` populado custava ~223k tokens de cache por chamada (~40-50s → ~10-15s).
- `enrich --operation re-embed --limit N --resume` é o caminho one-shot canônico de re-embedding; `edit --force-reembed` regenera um embedding sem mudar o body (G42/S9).
- Sem normalização silenciosa de dimensão (G42/C5): vetores divergentes falham com erro explícito em vez de truncamento ou preenchimento com zeros.
- Handler de sinais sem panic (G42/S8): o segundo sinal sai com 130 e ZERO I/O — elimina o SIGABRT em processos órfãos.
### OBRIGATÓRIO — G43: Adoção da Dimensionalidade em Toda Conexão
- `open_rw` E `open_ro` adotam `schema_meta.dim` em toda abertura de banco, então `remember` / `edit` / `recall` / `hybrid-search` operam na dimensionalidade do banco (antes do G43 usavam silenciosamente o default compilado contra bancos 384 pré-v1.0.79, gravando embeddings de dimensões misturadas invisíveis ao cosseno).
- `init` não carimba mais `dim=384`; `rename-entity` registra o tamanho real do vetor via o writer canônico `upsert_entity_vec`.
...
- Nova flag global `--extraction-backend llm|embedding|none|both` (padrão `llm`) seleciona o backend de extração. `llm` é o caminho LLM; `embedding` é um stub permanente desde a v1.0.79 (pipeline legado removido) que retorna erro de migração; `none` é um no-op; `both` roda os dois em paralelo e funde.
- O trait `ExtractionBackend` vive em `src/extract/` com quatro implementações concretas: `LlmBackend` (padrão), `EmbeddingBackend` (stub permanente desde a v1.0.79; pipeline legado removido), `NoneBackend` (no-op) e `CompositeBackend` (funde múltiplos backends em paralelo).
...
- NUNCA depender de `--enable-ner` para extração semântica de entidades; é somente URL-regex desde a v1.0.79 — use `--graph-stdin` com entidades curadas por LLM ou `ingest --mode claude-code|codex`
...
- USAR `--name-prefix <prefixo>` (v1.1.01) para prefixar o nome de cada memória ingerida
...
> **Autenticação:** OAuth é o ÚNICO fluxo de credenciais aceito. Chaves de API são PROIBIDAS.
> `--mode claude-code` lê OAuth de `~/.claude/.credentials.json` (Claude Pro/Max/Team).
> `--mode codex` lê autenticação de dispositivo via `codex auth login` (ChatGPT Pro da OpenAI).
> Definir `ANTHROPIC_API_KEY` ou `OPENAI_API_KEY` no ambiente ABORTA o spawn com `AppError::Validation` e exit code 1. A flag `--bare` (que também exigiria uma chave de API) foi REMOVIDA de todos os caminhos executáveis.


## Novidades na v1.0.69
### Enforcement OAuth-Only (MUDANÇA COMPORTAMENTAL — OBRIGATÓRIO)
- O spawn de `claude -p` e `codex exec` agora ABORTA com `AppError::Validation` se `ANTHROPIC_API_KEY` ou `OPENAI_API_KEY` estiverem presentes no ambiente. O fluxo OAuth (assinatura Claude Pro/Max, device auth do ChatGPT Pro) é o ÚNICO mecanismo de credencial aceito. Veja `docs/decisions/adr-0011-oauth-only-enforcement.md` para a justificativa completa.
- A flag `--bare` (que exige `ANTHROPIC_API_KEY` e desabilita OAuth) foi REMOVIDA de todo caminho executável. Ela aparece apenas em documentação explicando por que é proibida.
- `ANTHROPIC_API_KEY` e `OPENAI_API_KEY` estão INTENCIONALMENTE AUSENTES dos whitelists de `env_clear` em `claude_runner.rs`, `codex_spawn.rs` e `ingest_claude.rs`. Defesa em profundidade: mesmo se um refator futuro mover o guard OAuth-only, a variável nunca alcança o filho.
- Quatro novos testes (`#[serial_test::serial(env)]`) validam o conjunto canônico de flags e o comportamento de abort em `src/commands/claude_runner.rs:574-666` e `src/commands/codex_spawn.rs:684-758`.
- Migração: qualquer operador que dependa de `ANTHROPIC_API_KEY` ou `OPENAI_API_KEY` deve migrar para OAuth. A mensagem de erro é acionável e aponta para o fluxo de login OAuth.
### Correções de Proliferação de Processos (G28)
- `claude_runner::build_claude_command` SEMPRE passa `--strict-mcp-config --mcp-config '{}' --settings '{"hooks":{}}' --dangerously-skip-permissions` (sobrepondo o comportamento da v1.0.68 que citava o issue #10787 e evitava essas flags). O comentário da v1.0.68 estava errado: as flags estão presentes no código. O comentário da v1.0.69 alinha com a realidade e adiciona defesa em profundidade via guard OAuth-only.
- `run_claude` envia `SIGTERM` no timeout antes do `Child` ser descartado, para que processos filhos MCP não sobrevivam ao pai.
- Novo `src/reaper.rs` varre `/proc` no startup, mata qualquer órfão `claude`/`codex` com `PPID=1` e idade maior que 60 segundos. Invocado do `main` ANTES de qualquer trabalho.
- Novo `src/system_load.rs` fornece `load_average_one`, `ncpus` e `is_system_saturated`; `enrich` aborta o spawn quando `load_average_one() > 2 * ncpus` e a nova flag `--max-load-check` estiver setada (padrão true).
- `retry::CircuitBreaker` é integrado no loop de workers com `breaker.record(AttemptOutcome::HardFailure)`; o loop aborta após `--circuit-breaker-threshold` falhas consecutivas (padrão 5, 0 desabilita).
### Singleton Escopado por `db_hash` (G30)
- `lock::acquire_job_singleton(job_type, namespace, db_path, wait_seconds, force)`. O nome do arquivo de lock é `job-singleton-{tag}-{namespace_slug}-{db_hash}.lock` onde `db_hash` são os primeiros 12 caracteres hex de `blake3(canonicalize(db_path))`. Duas invocações concorrentes de `enrich` em bancos DIFERENTES não colidem mais.
- Novas flags CLI `--wait-job-singleton <SEGUNDOS>` e `--force-job-singleton` em `enrich` e `ingest`. A mensagem de erro que antes referenciava uma flag `--wait-job-singleton` inexistente agora é acionável.
### Helper de Spawn do Codex Unificado (G31+G32+G33)
- Novo `src/commands/codex_spawn.rs` (~700 linhas, 11 testes) possui o pipeline canônico de spawn, o parser JSONL e a validação de modelo ChatGPT Pro OAuth. Tanto `enrich --mode codex` quanto `ingest --mode codex` consomem o mesmo helper, eliminando a divergência que motivou o wrapper externo `~/.local/bin/codex-clean`.
- Lista branca de modelos ChatGPT Pro OAuth: `codex-auto-review`, `gpt-5.3-codex-spark`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.5`. A validação roda ANTES de o subprocesso ser iniciado; um modelo inválido retorna `AppError::Validation` listando os valores aceitos e o match fuzzy mais próximo.
- Novo subcomando de topo `codex-models --json` expõe a lista, contagem e modelo padrão. `codex-models --suggest <substring>` retorna o match mais próximo via substring ou Levenshtein.
- O schema JSON foi movido de `/tmp` para `paths::AppPaths::cache_dir().join("schemas")` para sobreviver a reboots e viver em um diretório trusted.
- O novo comando canônico inclui a flag de endurecimento OAuth-only `-c mcp_servers='{}'` e `--ask-for-approval never`.
### Portão de Preservação Jaccard (G29)
- Nova flag `--preserve-threshold <FLOAT>` em `enrich` (padrão 0.7). O novo módulo `src/preservation.rs` (10 testes) computa similaridade Jaccard trigrama entre corpos original e enriquecido. Se `score < threshold`, o corpo enriquecido é rejeitado com `EnrichItemResult::PreservationFailed` e NÃO é persistido.
- Idempotência via `blake3::hash`: quando `old_hash == new_hash`, o corpo é pulado com o motivo `"enriched body hash matches original (blake3:{hash}); idempotency skip"`. Reprocessar a mesma memória é seguro.
### Enum MemorySource (G29)
- Novo `src/memory_source.rs` (~180 linhas, 8 testes) define um enum type-safe dos cinco valores do CHECK-constraint: `Agent`, `User`, `System`, `Import`, `Sync`. `TryFrom<&str>` retorna `AppError::Validation` listando os valores aceitos.
- O guard runtime `pub fn validate_source(raw: &str) -> Result<&'static str, AppError>` é chamado de `memories::insert` e `memories::update`. Os call-sites existentes ainda usam `String` para compatibilidade binária; o enum é a fundação para a migração da v1.0.70.
### Flags de Endurecimento do FTS5 (G36)
- `optimize` faz pré-verificação da saúde do FTS5 via `check_fts_functional` ANTES de reconstruir. Um índice saudável não é mais reconstruído (economiza ~10 minutos em um banco de 4.3 GB).
- Novas flags: `--fts-dry-run` (exit 1 se reconstrução recomendada), `--fts-progress <N>` (polling em background da contagem de linhas de `fts_memories` a cada N segundos, padrão 30, 0 desabilita), `--yes` (reservada para compatibilidade futura).
- `OptimizeResponse` expõe `fts_rebuilt`, `fts_skipped_functional`, `fts_unhealthy` e `fts_rows_indexed` para observabilidade.
### Tratamento de Órfãos do vec (G39)
- Nova família de subcomandos `vec orphan-list`, `vec purge-orphan --yes --dry-run`, `vec stats --json`. `vec purge-orphan` purga TRÊS tabelas: `vec_memories`, `vec_entities` e `vec_chunks` em uma única transação.
- Novo hook em `src/commands/forget.rs:88-99` chama `memories::delete_vec` ANTES do soft-delete, prevenindo novos órfãos no estado estável.
### Endurecimento do Backup (G38)
- Padrões mudaram de `run_to_completion(100, 50ms)` para `run_to_completion(1000, 5ms)` (25x mais rápido em 4.3 GB).
...
- Novas flags `--names <NOME>` (delimitado por vírgula) e `--names-file <CAMINHO>` (um nome por linha, comentários `#` aceitos) em `enrich`. Operadores agora podem reprocessar uma única memória sem varrer o conjunto completo.
### Preflight e Fallback (G35)
- Novas flags `--preflight-check`, `--fallback-mode <codex|claude-code>` e `--rate-limit-buffer <SEGUNDOS>` em `enrich`. A sondagem de preflight emite um ping de 1 turno antes de varrer N candidatos; em rate limit do Claude, aborta com erro claro (ou troca para `--fallback-mode`).
### Aviso de Worker por Modo (G34)
- O aviso `llm_parallelism > 4` é condicional ao modo: Claude avisa em 5, Codex avisa em 17, Codex 5..16 fica silencioso (validado em 1161 itens, 0 falhas em produção).
...
- USAR `graph recompute-degree --json` (v1.1.01) para recalcular os graus armazenados das entidades em uma única transação; `--dry-run` pré-visualiza, `--namespace` delimita o escopo; envelope `{total, updated, zeroed, unchanged}`
...
## Novidades na v1.0.82 — Cinco Gaps Fechados
### OBRIGATÓRIO — pending (Fila de Checkpoint do `remember` em Três Estágios, ADR-0036)
...
- EXIT code 4 quando `show <id>` referencia um id inexistente; exit 1 para `--filter-status` inválido
### OBRIGATÓRIO — pending-embeddings (Fila de Retry, ADR-0040)
- USAR `sqlite-graphrag pending-embeddings list --json` para inspecionar a fila
- USAR `sqlite-graphrag pending-embeddings process --json` para reprocessar com o próximo backend em `--llm-backend`
- ESQUEMA: `docs/schemas/embedding-list.schema.json`
- COMBINAR com `--llm-backend codex,claude` para rotação automática de backend
### OBRIGATÓRIO — slots (Semáforo Cross-Process de LLM, ADR-0039)
...
- CAMPOS: `max_concurrency`, `acquired`, `waiting`, `held_by_pid[]`, `p50_wait_ms`, `p99_wait_ms`
- CRATE de lock é `fs4 = "0.9"` com `sync` (NÃO `fs2`); backend nativo é `fcntl(F_SETLK)` no Unix e `LockFileEx` no Windows
- COMBINAR com `--llm-max-host-concurrency N` para sobrescrever o teto padrão
### OBRIGATÓRIO — embedding (Saúde da Fila de Pendentes, ADR-0040)
- USAR `sqlite-graphrag embedding status --json` para contagens agregadas por status; desde a v1.1.01 o objeto `coverage` também reporta `memories_missing`, `entities_missing`, `chunks_missing`
...
### OBRIGATÓRIO — Incidente codex OAuth 401 (2026-06-14)
...
- P2: `enrich --operation re-embed` ganhou `--target memories|entities|chunks|all`; `enrich --status` reporta `scan_backlog` por alvo. P10: o predicado do re-embed agora também cobre dimensão divergente e blob vazio.
- P3: novo comando `graph recompute-degree` — recalcula os graus armazenados das entidades em transação única; aceita `--dry-run` e `--namespace`; envelope `{total, updated, zeroed, unchanged}`.
- P4: `reclassify-relation --literal-from` casa o valor gravado da relação de forma verbatim (bypassa a normalização do clap) — migra arestas legadas como `applies_to` com underscore.
- P5: `merge-entities` ganhou `--ids` e `--into-id`; `rename-entity` ganhou `--id` — manutenção de entidades por id além de por nome.
- P6: `health --json` ganhou `vec_memories_missing`, `vec_entities_missing`, `vec_chunks_missing` e `vec_*_coverage_pct`; o objeto `coverage` do `embedding status --json` ganhou `memories_missing`, `entities_missing`, `chunks_missing`.
- P7: `EntityType` agora tem `Deserialize` manual cuja mensagem de erro lista os 13 tipos canônicos.
- P11: erros de payload são tipados (`AppError::BodyTooLarge`, `AppError::TooManyChunks`); o exit 6 é preservado — apenas a mensagem do envelope ficou específica (informa o limite e o valor real).
...
- `enrich --max-attempts <N>` (padrão 8, range 1..=20) é o orçamento de retries Transient antes de `dead`. `enrich --status` é um relatório read-only JSON da fila (`unbound_backlog`, `scan_backlog` por operação, `queue_pending/done/failed/dead/skipped`, `eligible_now`, `waiting`) que NUNCA chama o LLM e NUNCA adquire o singleton — seguro para poll durante a execução; o `scan_backlog` (GAP-SG-77, v1.1.0) é o backlog real do banco por operação que um scan enfileiraria — elimina o falso `pending=0` para `entity-descriptions`/`body-enrich`/`re-embed`, e o `state` deriva o `pending-scan` dele.
